...
const Scrollbar = ({
    ...
    const handleTrackClick = useCallback((e) => {
        ...
        if(trackCurrent && contentCurrent) {
            ...
            // Next, figure out the distance between the top
            // of the track and the top of the viewport
            const target = e.target;
            const rect = target.getBoundingClientRect();
            const trackTop = rect.top;

            // We want the middle of the thumb to jump to where we clicked,
            // so we subtract half the thumb's height to offset the position
            const thumbOffset = -(thumbHeight / 2);

            // Find the ratio of the new position to the total content length
            // using the thumb and track values...
    ...
    const handleThumbMousemove = useCallback(
        (e) => {
            ...
            e.stopPropagation();
            if (isDragging) {
                const { contentScrollHeight, contentOffsetHeight } =
                    contentRef.current;

                // Subtract the current mouse Y position form where you started to get
                // the pixel difference in mouse position. Multiply by ratio of visible
                // content height to thumb height to scale up the difference for content scrolling.
                const deltaY =
        ...
        [isDragging, scrollStartPosition, initialScrollTop, thumbHeight]
    );
    ...
    }, [handleThumbMousemove, handleThumbMouseup, handleThumbMousedown]);
